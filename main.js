...
const GITHUB_TOKEN = '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';  // Ganti dengan token GitHub yang valid
const REPO_OWNER = 'Senx-wq';  // Ganti dengan pemilik repository
const REPO_NAME = 'Traz-INVICTUS';  // Ganti dengan nama repo
const FILE_PATH = 'Traz 2.4.json';  // Path file yang berisi database token
...
async function startBot() {
  try {
    ...
    if (!tokensData.tokens.includes(BOT_TOKEN)) {
      console.error(chalk.red.bold(`
╭───────────────────────
┃Hayo Token Kau Belum Terdaftar ❗
╰───────────────────────
      `));
